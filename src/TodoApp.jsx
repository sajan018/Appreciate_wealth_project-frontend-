...
const TodoApp = () => {
  ...
  useEffect(() => {
    ...
    const fetchTodos = async () => {
      try {
        const response = await axios.get('https://appreciate-wealth-project-backend-8.onrender.com/todos');
  ...
  const handleAddTodo = async () => {
    ...
    try {
      const response = await axios.post('https://appreciate-wealth-project-backend-8.onrender.com/todos', formData);
  ...
  const handleDeleteTodo = async (id) => {
    ...
    try {
      await axios.delete(`https://appreciate-wealth-project-backend-8.onrender.com/todos/${id}`);
  ...
  const handleUpdateTodo = async (todo) => {
    ...
    if (updatedTask) {
      try {
        await axios.put(`https://appreciate-wealth-project-backend-8.onrender.com/todos/${todo._id}`, { task: updatedTask, email: userEmail });
        ...
        <ul className='mt-10 flex flex-col gap-5'>
          {loading
            ...
            : todos.map((todo) => {
              ...
              return (
                <li key={todo._id} className='flex bg-white  flex-col md:flex-row items-center gap-4 md:gap-6 mb-4 px-2 sm:p-6 py-5 sm:px-10 border rounded-lg'>
                  <div className='flex gap-2 sm:gap-9 md:gap-12 md:px-5 justify-center items-center'>
                    {todo.image && (
                      <img
                        src={`https://appreciate-wealth-project-backend-8.onrender.com/uploads/${todo.image.split('/').pop()}`}
                        alt="todo"
                    ...
                    <div className='flex flex-col gap-3'>
                      ...
                      <span className='flex-1 font-sans text-xs sm:text-sm md:text-xl text-left'>{restOfTask.join(' ')}</span>
                    </div>
                  </div>
                  <div className='flex self-end mt-2 z-10 bg-lime-400 bg-opacity-45 filter backdrop-blur-lg px-6 rounded-3xl py-1 gap-2'>
                    {userEmail === todo.user_id && (
                      <>
                        <button onClick={() => handleUpdateTodo(todo)} className='w-8 h-8 rounded-full flex justify-center items-center bg-white'>
                          <RiEditLine className='text-xl' />
                        </button>
                        <button onClick={() => handleDeleteTodo(todo._id)} className='w-8 h-8 rounded-full flex justify-center items-center bg-white'>
